...
var D__Documents_GitHub_emojiButton_node_modules_tabbable = tabbable;
...
function focusTrap(element, userOptions) {
  ...
  function checkPointerDown(e) {
    ...
    if (config.clickOutsideDeactivates) {
      deactivate({
        returnFocus: !D__Documents_GitHub_emojiButton_node_modules_tabbable.isFocusable(e.target)
      });
      return;
    }
  ...
  function updateTabbableNodes() {
    var tabbableNodes = D__Documents_GitHub_emojiButton_node_modules_tabbable(container);
    ...
    showPreview(emoji) {
        this.emoji.innerHTML =
            this.options.style === 'native'
                ? emoji.emoji
                : twemoji_npm.parse(emoji.emoji, twemojiOptions);
...
class Emoji {
    ...
    render() {
        ...
        this.emojiButton.innerHTML =
            this.options.style === 'native'
                ? this.emoji.emoji
                : twemoji_npm.parse(this.emoji.emoji, twemojiOptions$1);
...
class EmojiContainer {
    ...
    render() {
        ...
        this.emojis.forEach(emoji => emojiContainer.appendChild(new Emoji(emoji, this.showVariants, true, this.events, this.options).render()));
...
class Search {
    constructor(events, i18n, options, emojiData, categories) {
        ...
        this.emojiData = emojiData.filter(e => e.version &&
    ...
    onKeyUp() {
        if (!this.searchField.value) {
            ...
            const searchResults = this.emojiData.filter(emoji => emoji.name
                .toLowerCase()
...
class VariantPopup {
    ...
    render() {
        ...
        this.popup.addEventListener('keydown', event => {
            if (event.key === 'ArrowRight') {
                this.setFocusedEmoji(Math.min(this.focusedEmojiIndex + 1, this.popup.querySelectorAll('.emoji-picker__emoji').length - 1));
            }
            else if (event.key === 'ArrowLeft') {
...
class EmojiArea {
    constructor(events, i18n$1, options) {
        this.events = events;
        this.i18n = i18n$1;
        this.options = options;
        ...
        this.addCategory = (category, emojis) => {
            ...
            this.emojis.appendChild(new EmojiContainer(emojis, true, this.events, this.options).render());
        };
        this.selectCategory = (category, focus = true) => {
            ...
            if (window && window["jQuery"]) {
                window["jQuery"](this.emojis).animate({ scrollTop: targetPosition }, 0.3);
            }
            else {
    ...
    render() {
        ...
        this.categories.forEach(category => this.addCategory(category, emojiCategories[category]));
        ...
        const firstEmoji = this.container.querySelectorAll('.emoji-picker__emoji')[0];
        firstEmoji.tabIndex = 0;
    ...
    getEmojiCount(category) {
        const container = this.emojis.querySelectorAll('.emoji-picker__container')[category];
...
class EmojiButton {
    ...
    buildPicker() {
        ...
        if (this.options.showSearch) {
            const searchContainer = new Search(this.events, this.i18n, this.options, emojiData.emoji, (this.options.categories || []).map(category => emojiData.categories.indexOf(category))).render();
        ...
        this.events.on(EMOJI, ({ emoji, showVariants }) => {
            if (emoji.variations &&
                showVariants &&
                this.options.showVariants) {
                variantPopup = new VariantPopup(this.events, emoji, this.options).render();
                if (variantPopup) {
                ...
                if (this.options.style === 'twemoji') {
                    this.publicEvents.emit('emoji', twemoji_npm.parse(emoji.emoji, twemojiOptions$2));
                }
                else {
    ...
    togglePicker(referenceEl, options = {}) {
        this.pickerVisible
            ? this.hidePicker()
    ...
    onDocumentKeydown(event) {
        if (event.key === 'Escape') {
            ...
            const searchField = this.pickerEl.querySelector('.emoji-picker__search');
...
export default window.EmojiButton;
