...
(function () {
	...
	*/
	...
	let newLoginPage = '<div class="app-signin-wrap mtd-signin-wrap">\
	...
	*/
	...
	const isApp = typeof require !== "undefined"; // may also return true on chromium-based clients like opera, edge chromium, and electron

	const isWin = navigator.userAgent.indexOf("Windows NT") > -1;
	const isMac = navigator.userAgent.indexOf("Mac OS X") > -1;
	const isiOS = navigator.userAgent.indexOf("iPhone OS") > -1; // Use standard macOS symbols instead of writing it out like on Windows

	const ctrlShiftText = isMac ? "⌃⇧" : "Ctrl+Shift+"; // We define these later. FYI these are jQuery objects.
	...
	function openSettings(openMenu) {
	  ...
	  for (var key in settingsData) {
	    ...
	    if (!settingsData[key].enum && settingsData[key].enabled !== false && settingsData[key].visible !== false) {
	      ...
	      let h2 = make("h2").addClass("mtd-version-title").html(( "Version " ) + version + " (Build " + buildId + ")");
	...
	function coreInit() {
	  ...
	  if (typeof $ === "undefined") {
	    try {
	      ...
	      console.error("jQuery failed. This will break approximately... everything."); //alert("ModernDeck was unable to find the page's jQuery runtime. This will result in application instability. Please notify @ModernDeck or @dangeredwolf of this issue immediately.");
	    }
	  }
