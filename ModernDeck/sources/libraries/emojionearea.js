...
function sliceFunc(string, start, end) {
  var accumulator = "";
  var character;
  var stringIndex = 0;
  var unicodeIndex = 0;
...
function unicodeSubstring(string, start, end) {
  ...
  if (realEnd == realStart) {
    ...
    return sliceFunc(string, realStart, realEnd);
  } else {
    return sliceFunc(string, realEnd, realStart);
  }
}
    ...
    function getTemplate(template, unicode, shortname) {
        ...
        template = template.replace('{name}', shortname || '')
            ...
            .replace('{uni}', unicode.toLowerCase().replace(/\-200d/g,"").replace(/\-fe0f/g,""));
        ...
        if (supportMode > 4) {
            defaultOptions.filters = {
                tones: {
                    ...
                    "nose baby boy girl man woman blond-haired_woman older_man older_woman " +
                ...
                smileys_people: {
                    ...
                    "busts_in_silhouette baby boy girl man woman blond-haired_woman older_man older_woman " +
    ...
    function getRecent () {
        console.log(localStorage.getItem("recent_emojis"));
        return localStorage.getItem("recent_emojis") || "";
    }
    function updateRecent(self, show) {
        console.log("updateRecent "+show);
        console.log(self);
        ...
        if (!self.recent || self.recent !== emojis || show) {
            if (emojis.length) {
                var skinnable = self.scrollArea.is(".skinnable"),
                    ...
                    scrollTop = $(".mtd-emoji-scroll-area").scrollTop();
                    if (show) {
                ...
                console.log(items);
                $(items).insertAfter(self.recentCategory.children(".mtd-emoji-category-title"));
...
// see https://github.com/Modernizr/Modernizr/blob/master/feature-detects/storage/localstorage.js
    ...
    function init(self, source, options) {
        ...
        self.emojiTemplate = '<img alt="{alt}" class="emojione">';
        self.emojiTemplateAlt = '<i class="mtd-emoji-code _{uni}"/>';
        ...
        var sourceValFunc = source.is("TEXTAREA") || source.is("INPUT") ? "val" : "text",
            ...
            button = self.button = div('button',
                div('button-open'),
                div('button-close')
            ).attr('title', options.buttonTitle).addClass("btn"),
            picker = self.picker = div('picker popover',
                div('wrapper',
                    filters = div('filters'),
                    (options.search ?
                    ...
                    scrollArea = div('scroll-area scroll-v',
        ...
        $.each(options.filters, function(filter, params) {
            ...
            do {
                ...
                items = shortnameTo(items,'<i class="emojibtn" role="button" data-name="{name}" title="{friendlyName}"><i class="mtd-emoji-code _{uni}"></i></i>').split('|').join('');

                category.html(items);
                $('<div class="mtd-emoji-category-title txt-mute"/>').text(params.title).prependTo(category);
            } while (--skin > 0);
        });
        ...
        attach(self, picker.find(".mtd-emoji-filter"), {click: "filter.click"});
        ...
        self.on("@filter.click", function(filter) {
            ...
            var headerOffset = categories.filter('[name="' + filter.data('filter') + '"]').offset().top,
                scroll = $(".mtd-emoji-scroll-area").scrollTop(),
        ...
        .on("@emojibtn.click", function(emojibtn) {
            ...
            let theEmoji = shortnameTo(emojibtn.data("name"), "{alt}");
            ...
            if (theInput.setSelectionRange) {
                ...
                console.log("sS "+sS);
                console.log("sE "+sE);
                //theInput.value = unicodeSubstring(theInput.value, 0, sS) + theEmoji + unicodeSubstring(theInput.value, sE);
        ...
        if (options.search) {
            ...
            .on("@search.keypress", function(hide) {
                var filterBtns = picker.find(".mtd-emoji-filter");
    ...
    function loadEmojione(options) {
        ...
        if (!cdn.isLoading) {
            if (!emojione || getSupportMode(detectVersion(emojione)) < 2) {
                ...
                $.ajax({
                    url: MTDBaseURL + "/sources/libraries/emojione.js",
                    dataType: "script",
                    cache: true,
                    success: function () {
                 ...
                 cdn.base = "https://abs.twimg.com/emoji/v2/72x72/";
            }
        }
